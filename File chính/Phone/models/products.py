...
class ProductProduct(models.Model):
    ...
    ncc_model=fields.Many2one('ncc.model',string='Supplier',required=True)


    ModelNumber=fields.Char(String='Model Number')
    pin_model=fields.Many2one('pin.model', string='Pin', required=True)

    TL_BH = fields.Many2one('warranty.model', string="Warranty period", required=True)
    bundled_gift = fields.Many2many('bundled.gift', string="Bundled gift", required=True)
    branch_model = fields.Many2one('branch.model', string="Branch", required=True)
    customer_object = fields.Many2many('customer.model', string="Customer object", required=True)
